...
class SignalManager {
   ...
   */
  static getDefault() {
    if (!this._singleton) this._singleton = new SignalManager();
...
class SignalTracker {
  ...
  track(obj, ...handlerIds) {
    if (_hasDestroySignal(obj)) this._trackDestroy(obj);
  ...
  untrack(obj) {
    ...
    if (destroyId) this._disconnectSignal(obj, destroyId);

    if (this._map.size === 0) this._removeTracker();
  }
