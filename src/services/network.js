...
const NMMenuItem = GObject.registerClass(
  {
    Properties: {
      'radio-mode': GObject.ParamSpec.boolean(
        'radio-mode',
        null,
        null,
  ...
  class NMMenuItem extends PopupMenu.PopupBaseMenuItem {
    get state() {
      return (
        this._activeConnection?.state ?? NM.ActiveConnectionState.DEACTIVATED
      );
    }
    ...
    // PopupMenu.PopupBaseMenuItem.destroy
    destroy() {}
  },
);
...
const NMSectionItem = GObject.registerClass(
  {
    Properties: {
      'use-submenu': GObject.ParamSpec.boolean(
        'use-submenu',
        null,
        null,
        GObject.ParamFlags.READWRITE,
...
const NMConnectionItem = GObject.registerClass(
  class NMConnectionItem extends NMMenuItem {
    ...
    activate() {
      ...
      if (this.radio_mode && this._activeConnection != null) return; // only activate in radio mode

      if (this._activeConnection == null)
        this._section.activateConnection(this._connection);
      else this._section.deactivateConnection(this._activeConnection);
...
const NMDeviceConnectionItem = GObject.registerClass(
  {
    Properties: {
      'device-name': GObject.ParamSpec.string(
        'device-name',
        null,
        null,
        GObject.ParamFlags.READWRITE,
  ...
  class NMDeviceConnectionItem extends NMConnectionItem {
    ...
    get name() {
      return this.radioMode ? this._connection.get_id() : this.deviceName;
    }
  },
);
...
const NMDeviceItem = GObject.registerClass(
  {
    Properties: {
      'single-device-mode': GObject.ParamSpec.boolean(
        'single-device-mode',
        null,
        null,
        GObject.ParamFlags.READWRITE,
  ...
  class NMDeviceItem extends NMSectionItem {
    constructor(client, device) {
      ...
      if (this.constructor === NMDeviceItem)
        throw new TypeError(
          `Cannot instantiate abstract type ${this.constructor.name}`,
        );
    ...
    _syncConnections() {
      ...
      const removed = [...this._connectionItems.keys()].filter(
        (conn) => !available.includes(conn),
      );

      for (const conn of removed) this._removeConnection(conn);

      for (const conn of available) this._addConnection(conn);
    }

    _getActivatableItem() {
      const [lastUsed] = this._itemSorter.itemsByMru();
      if (lastUsed?.timestamp > 0) return lastUsed;

      const [firstItem] = this._itemSorter;
      if (firstItem) return firstItem;

      console.assert(
        this._autoConnectItem.visible,
    ...
    activate() {
      ...
      if (this._activeConnection) this.deactivateConnection();
      else this._getActivatableItem()?.activate();
    }
    ...
    _addConnection(connection) {
      if (this._connectionItems.has(connection)) return;

      connection.connectObject(
        'changed',
        this._onConnectionChanged.bind(this),
        this,
      );
...
const WirelessNetwork = GObject.registerClass(
  ...
  class WirelessNetwork extends GObject.Object {
    static _securityTypes = Object.values(NM.UtilsSecurityType).sort(
      (a, b) => b - a,
    );
    ...
    get icon_name() {
      if (this._mode === NM80211Mode.ADHOC) return 'network-workgroup-symbolic';

      if (!this._bestAp) return '';
    ...
    get is_active() {
      if (this._accessPoints.has(this._device.activeAccessPoint)) return true;
    ...
    checkAccessPoint(ap) {
      if (!ap.get_ssid()) return false;

      const secType = this._getApSecurityType(ap);
      if (secType === NM.UtilsSecurityType.INVALID) return false;

      if (this._accessPoints.size === 0) return true;

      return (
        this._ssid.equal(ap.ssid) &&
        this._mode === ap.mode &&
     ...
     */
    addAccessPoint(ap) {
      if (!this.checkAccessPoint(ap)) return false;
      ...
      if (this._accessPoints.size === 0) {
        ...
        this._name =
          NM.utils_ssid_to_utf8(this._ssid.get_data()) || '<unknown>';
      ...
      if (wasActive !== this.is_active) this.notify('is-active');
    ...
    removeAccessPoint(ap) {
      const wasActive = this.is_active;
      if (!this._accessPoints.delete(ap)) return false;
    ...
    compare(other) {
      ...
      if (cmpConnections !== 0) return cmpConnections;
      ...
      if (cmpStrength !== 0) return cmpStrength;
    ...
    checkConnections(connections) {
      ...
      this._connections = connections.filter((c) =>
        aps.some((ap) => ap.connection_valid(c)),
      );
    }
    ...
    activate() {
      ...
      if (conn) {
        this._device.client.activate_connection_async(
          conn,
          this._device,
          null,
          null,
        ...
        const permission = Polkit.Permission.new_sync(
    ...
    _getApSecurityType(ap) {
      ...
      const bestType = WirelessNetwork._securityTypes.find((t) =>
    ...
    _updateBestAp() {
      const [bestAp] = [...this._accessPoints].sort(
        (a, b) => b.strength - a.strength,
      );

      if (this._bestAp === bestAp) return;
...
const NMWirelessNetworkItem = GObject.registerClass(
  class NMWirelessNetworkItem extends PopupMenu.PopupBaseMenuItem {
    ...
    _updateAccessibleName() {
      ...
      let signalStrengthString = _('Signal strength %s%%').format(
        this._network.signal_strength,
      );
      // translators: The first placeholder is the network name, the second and indication whether it is secure, and the last the signal strength indication
      // this.accessible_name = _('%s, %s, %s').format(
      //   this._label.text,
      //   secureString,
...
const NMWirelessDeviceItem = GObject.registerClass(
  {
    Properties: {
      'is-hotspot': GObject.ParamSpec.boolean(
        'is-hotspot',
        null,
        null,
        GObject.ParamFlags.READABLE,
  ...
  class NMWirelessDeviceItem extends NMSectionItem {
    constructor(client, device) {
      ...
      this.connect('destroy', () => {
        for (const net of this._networkItems.keys()) net.destroy();
      });
    }
    ...
    get icon_name() {
      ...
      switch (this.state) {
        ...
        case NM.ActiveConnectionState.ACTIVATED: {
          if (this.is_hotspot) return 'network-wireless-hotspot-symbolic';
    ...
    _addAccessPoint(ap) {
      ...
      let network = [...this._networkItems.keys()].find((n) =>
        n.checkAccessPoint(ap),
      );
    ...
    _removeAccessPoint(ap) {
      const network = [...this._networkItems.keys()].find((n) =>
        n.removeAccessPoint(ap),
      );

      if (!network || network.hasAccessPoints()) return;
    ...
    _updateItemsVisibility() {
      const { hasWindows } = false; // Main.sessionMode;
...
const NMToggle = GObject.registerClass(
  ...
  class NMToggle extends QuickMenuToggle {
    ...
    setClient(client) {
      ...
      if (this._client) this._loadInitialItems();
      this._sync();
    }
    ...
    *_getActiveItems() {
    ...
    _getPrimaryItem() {
      ...
      console.assert(!this.visible, `${this} should not be visible when empty`);
...
const NMVpnToggle = GObject.registerClass(
  class NMVpnToggle extends NMToggle {
    ...
    setClient(client) {
      ...
      this._client?.connectObject(
        'connection-added',
        (c, conn) => this._addConnection(conn),
        'connection-removed',
        (c, conn) => this._removeConnection(conn),
    ...
    _loadInitialItems() {
      ...
      for (const conn of connections) this._addConnection(conn);

      this._syncActiveConnections();
    }

    _syncActiveConnections() {
      const activeConnections = this._client
        .get_active_connections()
        .filter((c) => this._shouldHandleConnection(c.connection));

      for (const item of this._items.values()) item.setActiveConnection(null);
    ...
    _shouldHandleConnection(connection) {
      ...
      if (setting.get_master()) return false;
    ...
    _addConnection(connection) {
      if (this._items.has(connection)) return;

      if (!this._shouldHandleConnection(connection)) return;
...
const NMDeviceToggle = GObject.registerClass(
  class NMDeviceToggle extends NMToggle {
    ...
    _transformSubtitle(source) {
      ...
      if (subtitle === this.title) return null;
      return subtitle;
    }

    _loadInitialItems() {
      const devices = this._client.get_devices();
      for (const dev of devices) this._addDevice(dev);
      this._syncDeviceNames();
    }
    ...
    _syncDeviceItem(device) {
      if (this._shouldShowDevice(device)) this._ensureDeviceItem(device);
      else this._removeDeviceItem(device);
    }

    _deviceStateChanged(device, newState, oldState, reason) {
      if (newState === oldState) {
        console.info(
          `${device} emitted state-changed without actually changing state`,
        );
        return;
      }
      ...
      if (
        newState === NM.DeviceState.FAILED &&
    ...
    _ensureDeviceItem(device) {
      if (this._items.has(device)) return;
    ...
    _addDevice(device) {
      if (this._nmDevices.has(device)) return;

      if (device.get_device_type() !== this._deviceType) return;
    ...
    _removeDevice(device) {
      if (!this._nmDevices.delete(device)) return;
  ...
  class NMWirelessToggle extends NMDeviceToggle {
    constructor() {
      ...
      this.menu.connectObject('open-state-changed', (m, isOpen) => {
        if (isOpen) this._startScanning();
        else this._stopScanning();
      });
    ...
    async _scanDevice(device) {
      ...
      return new Promise((resolve) => {
        GLib.timeout_add(GLib.PRIORITY_DEFAULT, 1500, () => {
          if (device.lastScan === lastScan) return GLib.SOURCE_CONTINUE;
    ...
    async _scanDevices() {
      if (!this._client.wireless_enabled) return;

      this._scanningSpinner.play();

      const devices = [...this._items.keys()];
      await Promise.all(devices.map((d) => this._scanDevice(d)));
    ...
    _stopScanning() {
      if (this._scanTimeoutId) GLib.source_remove(this._scanTimeoutId);
    ...
    _shouldShowDevice(device) {
      ...
      if (device.state === NM.DeviceState.UNAVAILABLE) return true;
...
class CaptivePortalHandler extends Signals.EventEmitter {
  ...
  addConnection(name, path) {
    ...
    // );
    // notification.connect('destroy', () => this._notifications.delete(path));
  ...
  removeConnection(path) {
    ...
    // this._notifications
    //   .get(path)
    //   ?.destroy(MessageTray.NotificationDestroyedReason.SOURCE_CLOSED);
  ...
  _onNotificationActivated(path) {
    ...
    else Gio.AppInfo.launch_default_for_uri(this._checkUri, context);
  ...
  class Indicator extends SystemIndicator {
    ...
    async _getClient() {
      ...
      try {
        ...
        this.quickSettingsItems.forEach((toggle) => {
          this._configPermission.bind_property(
            'allowed',
            toggle,
            'reactive',
            GObject.BindingFlags.SYNC_CREATE,
    ...
    _syncMainConnection() {
      ...
      if (this._mainConnection) {
        this._mainConnection.connectObject(
          'notify::state',
    ...
    _syncConnectivity() {
      ...
      // if (!isPortal || Main.sessionMode.isGreeter) return;
...
const Network = GObject.registerClass(
  {
    Signals: {
      'network-update': { param_types: [GObject.TYPE_OBJECT] },
    },
  },
  class Network extends Extension {
    _init(params) {
      super._init(params);
    }
