...
`;
const rfkillManagerInfo = Gio.DBusInterfaceInfo.new_for_xml(
...
const BtClient = GObject.registerClass(
  {
    Properties: {
      ...
      "adapter-state": GObject.ParamSpec.enum(
        "adapter-state",
        null,
        null,
        GObject.ParamFlags.READABLE,
  ...
  class BtClient extends GObject.Object {
    _init() {
      ...
      this._proxy
        .init_async(GLib.PRIORITY_DEFAULT, null)
    ...
    get available() {
      ...
      return (
        this._proxy.BluetoothHasAirplaneMode &&
    ...
    get adapter_state() {
      if (this._predictedState !== undefined) return this._predictedState;
    ...
    toggleActive() {
      ...
      // a timeout
      setTimeout(
        () => this._client.notify("default-adapter-state"),
    ...
    async toggleDevice(device) {
      ...
      console.debug(
        `${connect ? "Connect" : "Disconnect"} device "${device.name}"`,
      );
      ...
      try {
        ...
        console.debug(
          `Device "${device.name}" ${connect ? "connected" : "disconnected"}`,
        );
      } catch (e) {
        console.error(
          `Failed to ${
            connect ? "connect" : "disconnect"
          } device "${device.name}": ${e.message}`,
        );
      }
    }
    ...
    *getDevices() {
      ...
      for (let i = 0; i < deviceStore.get_n_items(); i++) {
        ...
        if (device.paired || device.trusted) yield device;
      }
    }

    _queueDevicesChanged() {
      if (this._devicesChangedId) return;
...
const BluetoothToggle = GObject.registerClass(
  class BluetoothToggle extends QuickMenuToggle {
    _init(client) {
      ...
      // this._deviceSection.actor.bind_property(
      //   'visible',
      ...
      this._client.bind_property_full(
        "adapter-state",
        this,
        "icon-name",
        GObject.BindingFlags.SYNC_CREATE,
      ...
      this._client.connectObject(
        ...
        () => this._sync(),
        "device-removed",
        (c, path) => this._removeDevice(path),
        this,
      );
      ...
      this.menu.connect("open-state-changed", (isOpen) => {
        ...
        if (isOpen) this._reorderDeviceItems();
      });
    ...
    _updateDeviceVisibility() {
      // this._deviceSection.actor.visible = [...this._deviceItems.values()].some(
      //   (item) => item.visible,
      // );
    ...
    _sync() {
      ...
      for (const dev of devices) {
        ...
        if (this._deviceItems.has(path)) continue;
      ...
      if (nConnected > 1)
        ...
        //   nConnected,
        // ).format(nConnected);
        this.subtitle = `${nConnected} connected`;
      else if (nConnected === 1) this.subtitle = connectedDevices[0].alias;
    ...
    _getIconNameFromState(state) {
      switch (state) {
        ...
        default:
          console.warn(
            `Unexpected state ${GObject.enum_to_string(AdapterState, state)}`,
          );
          return "";
      }
    }
...
const BtIndicator = GObject.registerClass(
  class BtIndicator extends SystemIndicator {
    _init() {
      super._init();
      ...
      this._primaryIndicator = this.quickSettingsItems[0];
...
const Bluetooth = GObject.registerClass(
  {
    Signals: {
      "bluetooth-update": { param_types: [GObject.TYPE_OBJECT] },
    },
  },
  class Bluetooth extends Extension {
    _init(params) {
      super._init(params);
    }
    ...
    async enable() {
      ...
      try {
        this.indicator = new BtIndicator();
    ...
    sync() {
      ...
      this.emit("bluetooth-update", this);
    }
  },
);
...
// devices = Main.bluetooth.indicator.quickSettingsItems[0]._getSortedDevices()
// Main.bluetooth.indicator.quickSettingsItems[0]._client.toggleDevice(devices[0])
